...
interface ICustomerOpportunitySizeChartProps {
  ...
  statusOrder: string[];  // NEW PROP (for consistent status order in chart)
...
const CustomerOpportunitySizeByStatusChart: React.FC<ICustomerOpportunitySizeChartProps> = ({
  salesRecords, salesCustomers, statusOrder, statusColors // Update destructuring
}) => {
  ...
  useEffect(() => {
    ...
    }).sort((a, b) => b.total - a.total); // Sort by total size

    // Use statusOrder for consistent keys, filtering out statuses not in current data
    const currentStatusKeys = statusOrder.filter(s => statusSet.has(s));
    setStatusKeys(currentStatusKeys);

    // statusColors is now passed as a prop, no need to generate here
    setData(chartData);
  }, [salesRecords, salesCustomers, statusOrder, statusColors]); // Update dependencies
      ...
      <div style={{ width: "100%", height: "500px", overflowX: "auto" }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Legend />
            {statusKeys.map((status) => ( // Use statusKeys derived from statusOrder
                <Bar
                key={status}
                dataKey={status}
                stackId="a"
